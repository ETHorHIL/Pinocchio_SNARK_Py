...
def evaluate_in_exponent(powers_of_tau, poly):
    ...
    """
    # the ZK obfuscation of h (=p/t) increases the degree of the polynimal
    # compared to the non-zk version
...
# setup
def zk_SNARK_setup(L, R, _O, n_stmt):
    """
    ...
    # values required for ZK
    gl_t = g_l * ts
    gr_t = g_r * ts
    go_t = g_o * ts

    gl_a_t = g_l * (alpha_l * ts)
    gr_a_t = g_r * (alpha_r * ts)
    go_a_t = g_o * (alpha_o * ts)

    gl_b_t = g_l * (beta * ts)
    gr_b_t = g_r * (beta * ts)
    go_b_t = g_o * (beta * ts)

    ZK = [gl_t, gr_t, go_t, gl_a_t, gr_a_t, go_a_t, gl_b_t, gr_b_t, go_b_t]
...
# Prover
def zk_SNARK_prover(L, R, O_, LROpoly, n_stmt, proving_key, a):
    """
    ...
    # parse ZK part of proving key
    gl_t = proving_key[4][0]
    gr_t = proving_key[4][1]
    go_t = proving_key[4][2]

    gl_a_t = proving_key[4][3]
    gr_a_t = proving_key[4][4]
    go_a_t = proving_key[4][5]

    gl_b_t = proving_key[4][6]
    gr_b_t = proving_key[4][7]
    go_b_t = proving_key[4][8]

    # generate random delta for ZK
    delta_l = random_fp()
    delta_r = random_fp()
    delta_o = random_fp()
    ...
    obfuscate_p = (delta_r * L_big) + (delta_l * R_big) + \
        (delta_l * delta_r * t) - delta_o

    h = p/t + obfuscate_p
    assert p == (h - obfuscate_p) * t

    # assign provers variable to encrypted polynomials
    gLbig_at_s = sum([gl_to_li[k] * a[k] for k in range(n_stmt, n)],
                     G*0) + gl_t * delta_l
    gRbig_at_s = sum([gr_to_ri[k] * a[k] for k in range(n_stmt, n)],
                     G*0) + gr_t * delta_r
    gObig_at_s = sum([go_to_oi[k] * a[k] for k in range(n_stmt, n)],
                     G*0) + go_t * delta_o

    gLbig_at_s_shift = sum([gl_to_li_shift_a[k-n_stmt] * a[k]
                           for k in range(n_stmt, n)], G*0) + gl_a_t * delta_l
    gRbig_at_s_shift = sum([gr_to_ri_shift_a[k-n_stmt] * a[k]
                           for k in range(n_stmt, n)], G*0) + gr_a_t * delta_r
    gObig_at_s_shift = sum([go_to_oi_shift_a[k-n_stmt] * a[k]
                           for k in range(n_stmt, n)], G*0) + go_a_t * delta_o
...
def zk_SNARK_verifier(L, R, O_, m, n, verifier_key, a_stmt, pi):
    """
    U: the matrix m*n representing the problem equations
    CRS: the common reference string, babysnark_setup()[0]
...
def testingProof():
    ...
    n_stmt = 1
    # you can choose between these two circuits
    L, R, O_, a = use_babySNARK_paper_example(1, 1, 0)
    ...
    proving_key, verifier_key, LROpoly = zk_SNARK_setup(L, R, O_, n_stmt)
    ...
    # prover
    print("Proving...")
    pi = zk_SNARK_prover(L, R, O_, LROpoly, n_stmt, proving_key, a)
    print("pi lenght: ", len(pi))

    print("Verifying...")
    success = zk_SNARK_verifier(L, R, O_, m, n,
